...
interface PortfolioContextType {
  ...
  loading: boolean;
  error: string | null;
  saveData: (newData: any, passcode: string) => Promise<{ success: boolean; message: string; error?: string }>;
}
...
export const PortfolioProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const saveData = async (newData: any, passcode: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return { success: false, error: result.error || "Failed to save portfolio data" };
      }
  ...
  return (
    <PortfolioContext.Provider value={{ data, loading, error, saveData }}>
      {children}
