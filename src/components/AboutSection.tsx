...
const capabilities = [
  ...
  },
  {
    ...
    description: "Translating abstract ideas into intuitive page flows, information hierarchy, and polished visual rhythm.",
    icon: Sparkles,
  },
  {
    title: "Delivery Discipline",
    description: "From prototyping to implementation, I keep scope clear, move deliberately, and optimize for shippable outcomes.",
      ...
      <div ref={contentRef} className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
        <Panel className="motion-panel reveal-up p-6 md:p-8">
          <div className="panel-line space-y-6 pt-6">
            ...
            <div className="flex flex-wrap gap-3">
              {markers.map((marker) => (
                <span key={marker} className="data-pill transition-all duration-300 hover:border-primary/30 hover:bg-primary/8 hover:text-foreground">
                  {marker}
                </span>
              ))}
            </div>
            <div className="flex flex-col gap-3 sm:flex-row">
              <a href="#contact" className="action-button">Discuss a project</a>
        ...
        <div className="grid gap-5">
          {capabilities.map(({ title, description, icon: Icon }, i) => (
            <Panel key={title} className="motion-panel reveal-up p-6 group" style={{ transitionDelay: `${(i + 1) * 100}ms` } as React.CSSProperties}>
              <div className="flex items-start gap-4 text-left">
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-primary/25 bg-primary/10 text-primary transition-all duration-300 group-hover:bg-primary/20 group-hover:shadow-[0_0_20px_hsl(var(--primary)/0.15)]">
                  <Icon size={22} />
                </div>
